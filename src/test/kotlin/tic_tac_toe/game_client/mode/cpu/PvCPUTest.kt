...
class PvCPUTest {
    ...
    @Test
    fun testPlayCallsGetNewState() {
        val mode = PvCPU(mockIO, mockMI, ai)
    ...
    @Test
    fun testPlayWillRecurseWhenMoveIsInvalid() {
        val board = mapOf(1 to 1, 2 to 2, 3 to 1, 4 to 0, 5 to 1, 6 to 0, 7 to 0, 8 to 2, 9 to 2)
        val state = State(board, 1, false, 1)
    ...
    @Test
    fun testPlayWillRecurseIfGameIsNotComplete() {
        val acceptedMoves = mutableListOf<Int>()
        ...
        every { mockMI.makeMove(gameId = 1, location = capture(acceptedMoves), player = 1) } returns secondResponse
        val mode = PvCPU(mockIO, mockMI, ai)
        ...
        verifySequence {
            ...
            mockIO.showBoard(updatedState)
            mockMI.makeMove(1, acceptedMoves.first(), 1)
